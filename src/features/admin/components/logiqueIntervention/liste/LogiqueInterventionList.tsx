...
interface LogiqueInterventionListProps {
    items: LogiqueIntervention[];
    isLoading: boolean;
    onEdit: (item: LogiqueIntervention) => void;
    onDelete: (id: number) => void;
}

export const LogiqueInterventionList = ({ items, isLoading, onEdit, onDelete }: LogiqueInterventionListProps) => {
    return (
            ...
            <div className="overflow-x-auto overflow-y-auto max-h-[500px]">
                <table className="w-full border-collapse">
                    <thead className="bg-slate-50 border-b border-slate-200 sticky top-0 z-10">
                        <tr>
                            <th className="p-4 text-[10px] font-bold uppercase tracking-widest text-slate-500 text-left">Logiques d'intervention</th>
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {isLoading ? (
                            ...
                            <tr><td colSpan={2} className="p-12 text-center text-[10px] font-bold text-slate-400 uppercase tracking-widest">Aucune logique d'intervention trouvée</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
